...
Usage:
python val_ROI_video_tensorrt.py \
...
# Han test
python val_ROI_video_sperm_VSL_selection.py --video test-videoes/20241105_115426118.mp4 --output output-videoes/20241105_115426118_Sperm_VSL_Selection.mp4 --det_engine yolo_weights/best.engine --seg_model ADSCNet_sperm_ROINAHead_250707 --seg_engine model_fp32.engine --conf 0.25 

python val_ROI_video_sperm_VSL_selection.py --video E:\\MMRL-LAB-NAS\\Han-Workspace\\19201200-savedImg\\capture_20250808-225344.mp4 --output E:\\MMRL-LAB-NAS\\Han-Workspace\\19201200-savedImg\\results\\capture_20250808-225344_Sperm_VSL_Selection.mp4 --det_engine yolo_weights/best.engine --seg_model ADSCNet_sperm_ROINAHead_250707 --seg_engine model_fp32.engine --conf 0.25 

python val_ROI_video_sperm_VSL_selection.py --video E:\\MMRL-LAB-NAS\\Han-Workspace\\19201200-savedImg\\capture_20250808-225705.mp4 --output E:\\MMRL-LAB-NAS\\Han-Workspace\\19201200-savedImg\\results\\capture_20250808-225705_Sperm_VSL_Selection.mp4 --det_engine yolo_weights/best.engine --seg_model ADSCNet_sperm_ROINAHead_250707 --seg_engine model_fp32.engine --conf 0.25 

python val_ROI_video_sperm_VSL_selection.py --video E:\\MMRL-LAB-NAS\\Han-Workspace\\19201200-savedImg\\capture_20250808-230336.mp4 --output E:\\MMRL-LAB-NAS\\Han-Workspace\\19201200-savedImg\\results\\capture_20250808-230336_Sperm_VSL_Selection.mp4 --det_engine yolo_weights/best.engine --seg_model ADSCNet_sperm_ROINAHead_250707 --seg_engine model_fp32.engine --conf 0.25 

# xiangya test
python val_ROI_video_sperm_VSL_selection.py --video E:\\MMRL-LAB-NAS\\Han-Workspace\\0819\\2022_8_19_13_47_9.avi --output E:\\MMRL-LAB-NAS\\Han-Workspace\\0819\\results\\2022_8_19_13_47_9_Sperm_VSL_Selection.mp4 --det_engine yolo_weights/best.engine --seg_model ADSCNet_sperm_ROINAHead_250707 --seg_engine model_fp32.engine --conf 0.25 

python val_ROI_video_sperm_VSL_selection.py --video E:\\MMRL-LAB-NAS\\Han-Workspace\\0819\\2022_8_19_13_51_8.avi --output E:\\MMRL-LAB-NAS\\Han-Workspace\\0819\\results\\2022_8_19_13_51_8_Sperm_VSL_Selection.mp4 --det_engine yolo_weights/best.engine --seg_model ADSCNet_sperm_ROINAHead_250707 --seg_engine model_fp32.engine --conf 0.25 

# 20250813 Swim Up Using
python val_ROI_video_sperm_VSL_selection.py --video test-videoes\\swimUpUsing_5min.mp4 --output E:\\MMRL-LAB-NAS\\Han-Workspace\\20250813-SpermVideo-DeadMostly\\results\\swimUpUsing_5min_Sperm_VSL_Selection.mp4 --det_engine yolo_weights/best.engine --seg_model ADSCNet_sperm_ROINAHead_250707 --seg_engine model_fp32.engine --conf 0.25 
...
class SpermTracker:
    """Enhanced sperm tracking with VSL calculation"""
    def __init__(self, fps=30.0, vsl_window=30, boundary_margin=10):
        self.fps = fps
        self.vsl_window = vsl_window  # frames for VSL calculation
        self.boundary_margin = boundary_margin
        self.sperm_data = {}  # {track_id: SpermData}
        self.best_sperm_id = None
        self.frame_count = 0
        
    def update_sperm_data(self, active_tracks, frame_shape):
        """Update sperm tracking data and calculate VSL"""
        self.frame_count += 1
        current_frame_time = self.frame_count / self.fps
        
        # Update existing sperm data
        active_track_ids = set()
        for track in active_tracks:
            active_track_ids.add(track.id)
            
            if track.id not in self.sperm_data:
                self.sperm_data[track.id] = SpermData(track.id)
            
            sperm = self.sperm_data[track.id]
            
            # Add new position if trajectory exists
            if len(track.trajectory) > 0:
                pos = track.trajectory[-1]
                sperm.add_position(pos, current_frame_time)
                
                # Calculate VSL for this sperm
                sperm.calculate_vsl(self.vsl_window)
                
                # Check boundary constraints
                sperm.check_boundary_distance(frame_shape, self.boundary_margin)
        
        # Remove inactive tracks
        inactive_ids = set(self.sperm_data.keys()) - active_track_ids
        for track_id in inactive_ids:
            del self.sperm_data[track_id]
            if self.best_sperm_id == track_id:
                self.best_sperm_id = None
    
    def select_best_sperm(self):
        """Select the best sperm based on VSL and constraints"""
        valid_sperm = []
        
        for sperm_id, sperm in self.sperm_data.items():
            # Only consider sperm with sufficient tracking history
            if sperm.has_sufficient_data(self.vsl_window) and not sperm.near_boundary:
                valid_sperm.append((sperm_id, sperm.current_vsl))
        
        if not valid_sperm:
            self.best_sperm_id = None
            return None
        
        # Sort by VSL (descending order)
        valid_sperm.sort(key=lambda x: x[1], reverse=True)
        
        # Check if we need to update the best sperm
        if self.best_sperm_id is None:
            # No current best sperm, select the fastest
            self.best_sperm_id = valid_sperm[0][0]
        else:
            # Check if current best sperm is still valid
            current_best = self.sperm_data.get(self.best_sperm_id)
            if (current_best is None or 
                current_best.near_boundary or 
                not current_best.has_sufficient_data(self.vsl_window)):
                # Current best is invalid, select new one
                self.best_sperm_id = valid_sperm[0][0]
            else:
                # Check if there's a consistently faster sperm
                for sperm_id, vsl in valid_sperm:
                    if sperm_id != self.best_sperm_id:
                        challenger = self.sperm_data[sperm_id]
                        if challenger.is_consistently_faster(current_best, self.vsl_window):
                            self.best_sperm_id = sperm_id
                            break
        
        return self.best_sperm_id
    
    def get_best_sperm_data(self):
        """Get the data for the best sperm"""
        if self.best_sperm_id and self.best_sperm_id in self.sperm_data:
            return self.sperm_data[self.best_sperm_id]
        return None

class SpermData:
    """Data structure for individual sperm tracking"""
    def __init__(self, track_id):
        self.track_id = track_id
        self.positions = deque(maxlen=100)  # Store recent positions
        self.timestamps = deque(maxlen=100)  # Store corresponding timestamps
        self.vsl_history = deque(maxlen=30)  # Store VSL history
        self.current_vsl = 0.0
        self.near_boundary = False
        
    def add_position(self, position, timestamp):
        """Add a new position and timestamp"""
        self.positions.append(position)
        self.timestamps.append(timestamp)
    
    def calculate_vsl(self, window_size):
        """Calculate VSL (Average Linear Velocity) over the specified window"""
        if len(self.positions) < 2:
            self.current_vsl = 0.0
            return
        
        # Use recent positions within the window
        ...
        # Calculate linear distance between first and last position
        ...
        if time_diff > 0:
            self.current_vsl = linear_distance / time_diff  # pixels per second
        else:
            self.current_vsl = 0.0
        
        # Add to VSL history
        self.vsl_history.append(self.current_vsl)
    
    def check_boundary_distance(self, frame_shape, margin):
        """Check if sperm is too close to frame boundaries"""
        ...
        self.near_boundary = (
            current_pos[0] < margin or 
            current_pos[0] > width - margin or
            current_pos[1] < margin or 
            current_pos[1] > height - margin
        )
    
    def has_sufficient_data(self, min_frames):
        """Check if sperm has sufficient tracking data"""
        return len(self.positions) >= min_frames
    
    def is_consistently_faster(self, other_sperm, window_size):
        """Check if this sperm is consistently faster than another"""
        if len(self.vsl_history) < window_size or len(other_sperm.vsl_history) < window_size:
            return False
        
        # Compare recent VSL values
        my_recent_vsl = list(self.vsl_history)[-window_size:]
        other_recent_vsl = list(other_sperm.vsl_history)[-window_size:]
        
        # Count how many times this sperm was faster
        faster_count = sum(1 for i in range(len(my_recent_vsl)) 
                          if my_recent_vsl[i] > other_recent_vsl[i])
        
        # Consider consistently faster if faster in at least 80% of comparisons
        return faster_count >= (window_size * 0.8)
...
def draw_best_sperm_only(img, sperm_tracker, segmentation_results, roi_size=64):
    """Draw only the best sperm with fixed 64x64 detection box and segmentation"""
    # Copy image to avoid modifying the original
    result_img = img.copy()
    
    # Get best sperm data
    best_sperm = sperm_tracker.get_best_sperm_data()
    
    if best_sperm is None or not best_sperm.positions:
        return result_img
    
    # Get current position of best sperm
    current_pos = best_sperm.positions[-1]
    center_x, center_y = current_pos
    
    # Define bright green color for the best sperm
    best_color = (0, 255, 0)  # Bright green in BGR format
    
    # Draw fixed 64x64 detection box
    half_size = roi_size // 2
    ...
    # Draw bounding box
    cv2.rectangle(result_img, (x1, y1), (x2, y2), best_color, 3)
    
    # Draw sperm ID and VSL
    # label = f"Best ID:{best_sperm.track_id} VSL:{best_sperm.current_vsl:.1f}px/s"
    label = f"Best"
    ...
    cv2.rectangle(result_img, (x1, y1 - text_height - baseline - 5), 
                 (x1 + text_width, y1 - 5), best_color, -1)
    
    # Draw text
    cv2.putText(result_img, label, (x1, y1 - baseline - 5), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    
    # Apply segmentation overlay if available
    if segmentation_results and best_sperm.track_id in segmentation_results:
        masks, roi_box = segmentation_results[best_sperm.track_id]
...
def process_video(det_engine, seg_engine, video_path, output_path, det_class_names, 
                  ...
                  roi_size=64):
    """Process video with detection, tracking and segmentation - focus on best sperm only"""
    # Open video file
    ...
    # Initialize sperm tracker
    sperm_tracker = SpermTracker(fps=fps, vsl_window=30, boundary_margin=80)
    ...
    while True:
        ...
        # Update sperm tracking data
        sperm_tracker.update_sperm_data(active_tracks, frame.shape)
        
        # Select best sperm
        best_sperm_id = sperm_tracker.select_best_sperm()
        
        track_time = (time.time() - track_start_time) * 1000  # to milliseconds
        tracking_times.append(track_time)
        
        # 3. Segmentation on best sperm ROI only
        seg_start_time = time.time()
        
        # Store segmentation results
        segmentation_results = {}  # {track_id: (masks, roi_box)}
        
        # Process only the best sperm
        if best_sperm_id is not None:
            best_sperm = sperm_tracker.get_best_sperm_data()
            
            if best_sperm and best_sperm.positions:
                # Get latest position
                center_x, center_y = best_sperm.positions[-1]
                ...
                if roi.shape[0] > 0 and roi.shape[1] > 0:
                    try:
                        ...
                        # Store results
                        segmentation_results[best_sperm_id] = (masks, roi_box)
                        total_segmentations += 1
                    except Exception as e:
                        print(f"Error processing segmentation for best sperm {best_sperm_id}: {e}")
        
        seg_time = (time.time() - seg_start_time) * 1000  # to milliseconds
        segmentation_times.append(seg_time)
        
        # 4. Visualize results - only best sperm
        result_frame = draw_best_sperm_only(
            frame, sperm_tracker, segmentation_results, roi_size
        )
        ...
        if show_preview:
            ...
            cv2.imshow('Best Sperm ROI Detection, Tracking & Segmentation', preview_frame)
    ...
    if detection_times:
        ...
        report = f"""
==================================================
TensorRT Best Sperm ROI Video Processing Performance Report:
==================================================
Video file: {video_path}
Output file: {output_path}
Total frames: {frame_count}
Total detections: {total_detections}
Total segmentations (best sperm only): {total_segmentations}
...
Note: Only the best sperm (highest VSL) is displayed with segmentation.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='YOLOv8 TensorRT Best Sperm ROI Video Processing with Segmentation')
    parser.add_argument('--video', type=str, required=True, help='Input video file path')
    parser.add_argument('--output', type=str, help='Output video file path (default adds _best_sperm suffix)')
    ...
    if args.output is None:
        ...
        args.output = str(video_path.with_stem(f"{video_path.stem}_best_sperm").with_suffix('.mp4'))
